...
const addUserTransactions = async (transaction) => {
  try {
    AsyncStorage.getItem("@user_info").then((user) => {
      ...
      user.totalSpent = parseInt(transaction.amount);

      if (transaction.type === TYPE.necessities)
        user.necessitiesTransactions += parseInt(transaction.amount);
      if (transaction.type === TYPE.entertainment)
        user.entertainmentTransactions += parseInt(transaction.amount);
      if (transaction.type === TYPE.personal)
        user.personalTransactions += parseInt(transaction.amount);
